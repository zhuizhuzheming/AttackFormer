...
Link:[https://sandbox.zenodo.org/records/492828](https://doi.org/10.5072/zenodo.492828)
## Research Motivation
...
I identify a critical gap in current literature: the absence of a closed-loop generative framework that treats open-source guard models not merely as black-box evaluators, but as differentiable signal sources for iterative policy improvement. Specifically, guard models emit rich intermediate representations—confidence scores, risk embeddings, and explanatory hidden states—that are typically discarded after binary classification. This represents an untapped resource for guiding adversarial generation.
To address these limitations, I propose AttackFormer, a diffusion-based iterative guard amplification framework. It leverages guard hidden states as cross-attention conditioning signals throughout generation, introduces learnable gating to accumulate guard signals across multiple rounds, and combines semantic-preserving diffusion with an adaptive multi-component reward function. By framing adversarial prompt generation as a reinforcement learning problem with guard-driven guidance, AttackFormer establishes a principled foundation for scalable, realistic red-teaming of next-generation LLM safety systems.

<img width="2387" height="3176" alt="image" src="https://github.com/user-attachments/assets/ad0ed266-4406-4e68-b47d-b908b3e51cc6" />
